...
function loadFrom(package, folder, opts) {
  ...
  for(let file of files) {
    if(file.endsWith('.js') || file.endsWith('.ts')) {
      ...
      if(cmdInst.inspectPackage(package)) {
        console.log(`Adding new command ${cmdInst.name}`);
        opts[cmdInst.name] = cmdInst;
      }
    }
  }
}
...
class PackagePrompter {
  ...
  async chooseOptions() {
    ...
    questions.source = async (answers, input) => {
      var fuzzyResult = fuzzy.filter(input, Object.keys(this.options));
  ...
  async getKey() {
    ...
    const options = [...Object.keys(this.pkg), "Cancel"];
    questions.pageSize = 4;
    questions.source = async (answers, input) => {
      var fuzzyResult = fuzzy.filter(input, options);
      return fuzzyResult.map(el => el.original || el);
    };
